...
def build_chunk(start_pg, end_pg):
    ...
    for p_num in range(start_pg, end_pg + 1):
        ...
        for line in lines:
            line_str = line.strip()
            if not line_str or line_str.startswith("---") or line_str.startswith("صفحه"):
                continue
            if len(line_str) < 40 and not line_str.endswith("."):
                body_html += f'<h2 class="chapter-heading">{line_str}</h2>\n'
            else:
...
print("Updated all 38 10-page responsive HTML chunks with ORCID iD badge.")
